...
`apps/web/.eslintrc.json` (`{"extends": "next/core-web-vitals"}`) and `eslint`/`eslint-config-next` are already installed and committed (`eslint@10.8.1`, `eslint-config-next@16.3.0`, matching the `next@16.3.0` major — bumped together during the Next.js 16 upgrade). If ESLint ever needs reinstalling, don't let `next lint`'s interactive setup auto-install — it defaults to `pnpm` on this machine, which can't resolve the workspace-internal `@finops-lab/shared` package and fails with a registry 404. Install via `npm install -D eslint@^10 eslint-config-next@^16 -w apps/web` instead, pinned to match whatever `next` major is currently installed.
