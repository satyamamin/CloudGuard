...
`docs/Architecture.png`, `docs/Architecture Addition.png`, and the multi-tenant framing in `docs/Tech Stack.png` describe an earlier, now-superseded shared-backend design (Key Vault for customer secrets, Service Bus, RLS-per-tenant Postgres) — only trust those images for the **language/framework** choices (Next.js, NestJS, Prisma, Tailwind/shadcn, TypeScript-everywhere), not topology or auth.
...
`docs/Internal integration secret.txt` contains a live Notion integration token in plaintext. Do not read its contents into context, echo it, or reference it in code/docs. Flag to the user rather than acting on it — the token should be revoked/rotated and moved to a proper secret store.

Separately: an ARM/Bicep deployment `output` (the customer's API key, generated by `main.bicep`) is **not actually ephemeral** — it stays visible in the resource group's Deployment history to anyone with read access, despite `docs/connect-azure.md`'s "shown once" framing. This is a known, accepted v1 gap (see `infra/bicep/README.md`), not a bug to silently fix.
