...
// Thin fetch wrapper around the 5 endpoints exposed by a customer's deployed
// backend instance (see docs/connect-azure.md). Every call here runs
// server-side (Route Handlers) so the API key never reaches the browser.
class BackendClient {
  constructor(
